...
/**
 * matches - a temporary array that at first contains every object of books array 
 *           but changes according to the filters applied by the user.
 * 
 * page - a variable that starts at 1 and increases by 1 or is reset based on 
 *        the state of the app.
 * 
 * range - an array that with two indexes, index 0 starts at 0 and index 1 starts at the
 *         BOOKS_PER_PAGE value which is 36, both indexes either increase by 36 or is reset 
...
/**
 * checks if books array is present and books array is an array otherwise throws an 
 * error, then checks if range array is present and range array length is greater than
 * two otherwise throws an error.
 */
...
/**
 * an object literal for all the query selectors used throughout the script.
 */
const html = {
...
/** 
 * an object with two different css values to be assigned to style properties based on chosen day/night setting.
 */
const css = {
...
/**
 * an event handler that first increases page by 1, range index 0 is increased by 36, and range index 1 is increased by 36,
 * then the createHTMLListItems function and the calcButtonRemainingBooks function is called.
 */
 ...
 * then a preview id variable is created and assigned the the current node's dataset preview id within the loop, then a nested for loop
